...
export async function loadEmails(append = false) {
  ...
  try {
    ...
    _loading = false;
    try { _renderPendingApprovals(); } catch (_) {}
  }
}

// ── Agent send-confirm: pending-approval banner above the inbox list ──
// When the email-safety gate (agent_email_confirm) is on, the agent stages
// outbound emails as scheduled_emails rows with status='agent_draft'. These
// come from the local DB, so the banner renders even when the IMAP inbox
// itself fails to load. Approve → POST /pending/{id}/approve (sends now);
// Reject → DELETE /pending/{id}.
async function _renderPendingApprovals() {
  const list = document.getElementById('email-list');
  if (!list || !list.parentNode) return;
  let box = document.getElementById('email-pending-approvals');
  if (!box) {
    box = document.createElement('div');
    box.id = 'email-pending-approvals';
    box.style.cssText = 'margin:0 0 10px;display:none;';
  ...
  } catch (_) { box.style.display = 'none'; box.innerHTML = ''; return; }
  if (!pending.length) { box.style.display = 'none'; box.innerHTML = ''; return; }
  box.style.display = '';
  box.innerHTML =
    `<div style="display:flex;align-items:center;gap:6px;padding:7px 10px;border:1px solid var(--red);border-bottom:none;` +
    `border-radius:8px 8px 0 0;background:color-mix(in srgb, var(--red) 13%, transparent);font-size:12px;font-weight:600;color:var(--red);">` +
    `<span>✋ ${pending.length} email${pending.length > 1 ? 's' : ''} awaiting approval</span></div>` +
    `<div id="email-pending-rows" style="border:1px solid var(--red);border-top:none;border-radius:0 0 8px 8px;overflow:hidden;"></div>`;
  const rows = box.querySelector('#email-pending-rows');
  pending.forEach((p, i) => rows.appendChild(_pendingApprovalItem(p, i === pending.length - 1)));
}

function _pendingApprovalItem(p, isLast) {
  const row = document.createElement('div');
  row.style.cssText = `padding:8px 10px;background:var(--panel);${isLast ? '' : 'border-bottom:1px solid var(--border);'}`;
  const preview = String(p.body || '').replace(/\s+/g, ' ').trim().slice(0, 120);
  row.innerHTML =
    `<div style="font-size:11px;opacity:.7;margin-bottom:2px;">To: ${_esc(p.to_addr || '')}</div>` +
    `<div style="font-size:13px;font-weight:600;margin-bottom:2px;">${_esc(p.subject || '(no subject)')}</div>` +
    `<div style="font-size:12px;opacity:.7;margin-bottom:8px;">${_esc(preview)}${preview.length >= 120 ? '…' : ''}</div>` +
    `<div style="display:flex;gap:6px;">` +
    `<button class="ep-approve" style="background:var(--green);color:#0b2018;border:none;border-radius:6px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;">Approve &amp; send</button>` +
    `<button class="ep-reject" style="background:transparent;color:var(--red);border:1px solid var(--red);border-radius:6px;padding:5px 12px;font-size:12px;font-weight:600;cursor:pointer;">Reject</button>` +
    `</div>`;
  const approve = row.querySelector('.ep-approve');
  const reject = row.querySelector('.ep-reject');
  const fail = (msg) => import('./ui.js').then(m => m.showError && m.showError(msg)).catch(() => {});
  approve.addEventListener('click', async () => {
    approve.disabled = reject.disabled = true; approve.textContent = 'Sending…';
    try {
      const r = await fetch(`${API_BASE}/api/email/pending/${encodeURIComponent(p.id)}/approve`, { method: 'POST', credentials: 'same-origin' });
      const j = await r.json();
      if (!j.success) throw new Error(j.error || 'approve failed');
      _renderPendingApprovals();
    } catch (_) {
      approve.disabled = reject.disabled = false; approve.textContent = 'Approve & send';
      fail('Could not approve the email.');
    }
  });
  reject.addEventListener('click', async () => {
    approve.disabled = reject.disabled = true; reject.textContent = 'Rejecting…';
    try {
      const r = await fetch(`${API_BASE}/api/email/pending/${encodeURIComponent(p.id)}`, { method: 'DELETE', credentials: 'same-origin' });
      const j = await r.json();
      if (!j.success) throw new Error(j.error || 'reject failed');
      _renderPendingApprovals();
    } catch (_) {
      approve.disabled = reject.disabled = false; reject.textContent = 'Reject';
      fail('Could not reject the email.');
    }
  });
  return row;
}
